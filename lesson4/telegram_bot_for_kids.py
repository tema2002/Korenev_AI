...
from gpt_handler import rewrite_for_kids, process_text_and_generate_image
...
from gpt_handler import process_text_and_generate_image
...
def handle_text(message):
    ...
    try:
        rewritten_text, image_prompt, image = process_text_and_generate_image(original_text, prompt, config.STABILITY_API_KEY)
        
        # Отправляем отрерайченный текст
        bot.reply_to(message, rewritten_text)
        
        # Сохраняем изображение во временный файл и отправляем его
        image_path = f"./temp_image_{user_id}.png"
        image.save(image_path)
        with open(image_path, 'rb') as photo:
            bot.send_photo(message.chat.id, photo, caption=f"Изображение, сгенерированное на основе текста.\nПромпт: {image_prompt}")
        os.remove(image_path)  # Удаляем временный файл
        
    except Exception as e:
        bot.reply_to(message, f"Произошла ошибка при обработке вашего сообщения: {str(e)}")
